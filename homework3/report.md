...
經由多重測試案例的驗證，包括空多項式、單項式、複雜多項式、邊界條件，乃至於加減運算結果為零的特殊情況，本程式皆能產出與預期一致的正確輸出。此一嚴謹的測試流程
，充分保證了程式運算邏輯的正確性與可靠度。
在記憶體管理方面，本程式採用鏈結串列搭配可用節點清單的策略，有效地避免了頻繁配置與釋放節點所造成的效能瓶頸。此舉不僅提升了運算效率，更顯著降低了記憶體碎片化的風險。
本程式採用物件導向的設計思維，架構清晰且模組化，有利於後續的維護與功能擴充。透過迭代器提供的標準化鏈結串列遍歷方式，多項式運算的邏輯得以簡化並易於擴展。整體而言
，本程式兼顧了效率與可讀性，極具進一步開發潛力，例如加入微分、多項式除法或快速冪次計算等功能。
